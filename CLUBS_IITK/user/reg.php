...
<?php 
    ...
    $username=$_SESSION['user'];  // echo"$username";
                 ...
                 $email = $cdrow[2]; 
                 $name =   $cdrow[3]; 
    $sqlc = "SELECT * FROM tbl_regs WHERE name = '$name' AND event = '$title'";
             $resc = mysqli_query($conn, $sqlc);
             $countc = mysqli_num_rows($resc);
             //echo $countc;
             if($countc>0){
                echo "<ol align='left'> <font color=red size='6pt'> You already registered to the event</font>";
             }
?>

<!-- Event sEARCH Section Starts Here -->
<section class="u" style="background-color:#55aacc">
    <div class="container">
